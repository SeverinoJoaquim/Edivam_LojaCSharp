...
namespace Setup.Formularios
{
    public partial class FrmCadPessoa : Form
    {
        ...
        private void BtnSalvarEndereco_Click(object sender, EventArgs e)
        {
            ...
            {
                Geral.Erro("O campo CEP está incompleto");
                TxtCEP.Focus();
                return;
            }
            ...
            {
                idBairro = CmbBairro.SelectedValue.ToString();
            }
            ...
            {
                idCidade = CmbCidade.SelectedValue.ToString();
            }
        }
...
//INICIO CIDADE/ESTADO
        ...
        {
            string sql = "SELECT a.ESTADO_ID, a.ESTADO FROM ESTADO a ORDER BY a.ESTADO";
            CmbEstado.DataSource = BD.Buscar(sql);
        ...
        private void TxtCEP_Leave(object sender, EventArgs e)
        {
            ...
            if(TxtCEP.Text.Length < 9)
            {
                Geral.Erro("Perdão, mas este CEP está, incompleto!");
                return;
            }
            ...
            try
            {
                CmbLogradouro.Text = ds.Tables[0].Rows[0]["logradouro"].ToString();
                TxtComplemento.Text = ds.Tables[0].Rows[0]["complemento"].ToString();
                CmbBairro.Text = ds.Tables[0].Rows[0]["bairro"].ToString();
                CmbCidade.Text = ds.Tables[0].Rows[0]["localidade"].ToString();
                CmbEstado.Text = ds.Tables[0].Rows[0]["uf"].ToString();
            }
            catch
            {
                CmbLogradouro.Text = "";
        ...
        private void CmbEstado_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string sql = "SELECT a.SIGLA FROM ESTADO a WHERE a.ESTADO = '" + CmbEstado.Text + "'";
            LblSigla.Text = BD.Buscar(sql).Rows[0]["sigla"].ToString();
        }

        private void BtnAddEstado_Click(object sender, EventArgs e)
        {
            FrmCadEstado frm = new FrmCadEstado();
            //frm.TxtEstado.Focus(); O foco não está no campo TxtEstado
            frm.ShowDialog(); 
        }

        private void CmbBairro_Leave(object sender, EventArgs e)
        {
            CmbBairro.SelectedIndex = CmbBairro.FindStringExact(CmbBairro.Text);
        }

        private void CmbCidade_Leave(object sender, EventArgs e)
        {
            CmbCidade.SelectedIndex = CmbCidade.FindStringExact(CmbCidade.Text);
        }
    }
}
