...
namespace Setup.Formularios
{
    public partial class FrmCadPessoa : Form
    {
        ...
        private void BtnSalvarPessoa_Click(object sender, EventArgs e)
        {
            ...
            string data= BD.CvData(TxtNascimento.Text);
            if (data != "null")
                data = "'" + data + "'";
            ...
            string sql = "SELECT p.ID FROM PESSOA_SALVAR(" + id + ", '" + TxtNome.Text + "', '" + TxtCodigo.Text + "', " + cpf + ", " + cnpj + ", " + sexo + ", " + data + ") p";
            TxtIdPessoa.Text = BD.Buscar(sql).Rows[0][0].ToString();
        }
